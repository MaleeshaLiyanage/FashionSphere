...
const { lowStockEmailTemplate } = require("../emails/low-stock");
  ...
  try {
    ...
    if (checkout.isPaid && !checkout.isFinalized) {
      ...
      if (lowStockProducts.length > 0) {
        ...
        const emailTemplate = lowStockEmailTemplate(lowStockProducts);
        const email = process.env.NOTIFY_EMAIL;

        // send email to user
        sendEmail(email, "Low Stock Alert", emailTemplate, undefined);
      }
